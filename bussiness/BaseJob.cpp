 ...
 * 对消息进行解析，解析出消息类型
 */
bool BaseJob::prase_message(std::string message_content)
/*
...
{
    std::string string_message = (char *)jobdata;/*消息*/
    Json::Reader reader;
    Json::Value json_message;
    /*解析string字符串为json对象*/
    if(!reader.parse(string_message, json_message))
        /*解析出错*/
    {
    ...
    case message_register:
        if(!register_message(message_content))
            /*注册消息失败*/
        {
